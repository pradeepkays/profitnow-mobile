...
export default function SupportTicket(props: AppScreenProps) {
  ...
  const getTicketsList = () => {
    fetch(
      `${API.supportingTicket}?limit=100&contact_id=${props.navigation.state.params.id}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
      ...
      .then(async (response) => {
        setSearchFilter(false)
        console.log('response::>>LL: ', JSON.stringify(response))
        if (response) {
          setIsLoading(false)
          if (response.data) {
  ...
  const filterTicketsData = () => {
    ...
    let tempArr = `?contact_id=${props.navigation.state.params.id}&`
    if (subject.length > 0) {
            ...
            >
              ...
              {searchFilter && (
                <TouchableOpacity onPress={() => resetAllFilters()}>
                  <Text style={[styles.resetText, { marginTop: 8 }]}>
                    RESET
                  </Text>
                </TouchableOpacity>
              )}
